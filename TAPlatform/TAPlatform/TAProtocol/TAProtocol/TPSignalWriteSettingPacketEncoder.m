...
{
    DDLogVerbose(@"in TPSignalSettingPacket \n");
 ...
 //   TPSignalSettingPacket.data[1] = value_byte[1];
    //the length of 'value' is 8(data from NSInteger),so use 'sizeof(t_setting_packet.data)' instead
    memcpy(t_setting_packet.data,value_byte,sizeof(t_setting_packet.data));
...
{
    // TODO length equal
    DDLogVerbose(@"check body length with TPSignalSettingPacket_t %lu \n",(unsigned long)body.length);
